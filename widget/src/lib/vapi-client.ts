...
export class VapiClient {
  ...
  constructor() {
    // Get config from window (set by widget initialization)
    const config = (window as any).__COURTNEY_WIDGET_CONFIG__ || {};
    this.publicKey = config.publicKey || '';
    this.assistantId = config.assistantId || '';

    if (!this.publicKey || !this.assistantId) {
      console.warn('[VAPI Warning] Missing credentials. Please configure CourtneyWidget with publicKey and assistantId');
    }
  }
